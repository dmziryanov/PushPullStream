...
namespace Solution
{
    ...
    {
        public PushStreamImpl()
        {
          //  _position = Owner.Buffer.Length;
        }
        ...
        public override async Task WriteAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken)
        {
            ...
            await Task.Run(() =>
            {
                ...
                Buffer.BlockCopy(buffer, 0, Owner.Buffer, offset, count);
                _position += count;
            }, cancellationToken);
        }
    }
    ...
    public class PullStreamImpl : StreamBase
    {
        ...
        public override async Task<int> ReadAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken)
        {
           ...
           res = await Task.Run(() =>
           {
               ...
               {
                   Buffer.BlockCopy(Owner.Buffer, 0, buffer, offset, count);
            ...
            int delayDurationInMilliseconds = 50)
        {
            using (var p2p = PushToPullFactory.Invoke())
            {
                ...
                var writerTask = Task.Run(async () =>
                {
                    ...
                    {
                        var chunkSize = (int)Math.Min(Random1.Next(1 + maxChunkLength), length - p2p.PushStream.Position);
                        Random1.NextBytes(buffer);
                        AccumulateCrc(buffer, buffer.Length, ref inputCrc);
                        if (Random1.NextDouble() < pushDelayProbability) await Task.Delay(delayDurationInMilliseconds);
